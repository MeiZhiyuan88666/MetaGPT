...
file = pd.read_json(f_MemeCap_path)
co_path = "Meta_Rec.xlsx"
...
meta_rec_question = "Please identify all metaphorical expressions in the meme. For each metaphor, extract the source domain and target domain as a pair." \
                    "Follow the format strictly as below:\n" \
                    "-Source domain–target domain pair 1: (source, target)\n" \
                    "-Source domain–target domain pair 2: (source, target)"

for i in range(len(file)):
    image_name = file.loc[i, 'img_fname']
    cap_gt = file.loc[i, 'meme_captions']
    Meta_gt = file.loc[i, 'metaphors']

    if image_name in before_data:
        print(f"当前数据: {image_name} 已经被评测过, 跳过")
        continue
    print(i, '-当前处理图片：', image_name)

    prompts = f"{meta_rec_question}"
    qs = prompts
    ...
    image_file = os.path.join(image_path_MemeCap, image_name)
